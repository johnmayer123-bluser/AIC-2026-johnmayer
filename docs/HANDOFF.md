...
- C00 LoveDA两阶段迁移研究线代码就绪。该线使用外部数据，所有输出强制research-only且禁止AIC提交；完整说明见`docs/C00_LOVEDA_RESEARCH_GUIDE.md`。
...
1. 冻结B00为当前合规官方最佳；不覆盖A00-B00任何输出。
2. 优先完成或确认B01官方数据-only对照，再决定合规B线下一改动。
3. C00仅作独立学术研究。先同步代码，再下载并核验LoveDA官方Train/Val；不下载LoveDA
   Test，不读取AIC测试，不生成提交ZIP。
4. C00先运行LoveDA 8类阶段，再通过`--init-checkpoint --research-only`运行官方数据研究
   阶段；启动报告必须只重新初始化两个classifier参数。
5. C00只能与B01在同一A00 split的验证集上作研究比较，不能提交排行榜、集成或替代B00。
6. 工作区`run_d03.ps1`和`tools/audit_label_consistency.py`为用户未跟踪文件，本次未修改或删除。
