...
- B00 DINOv3 ViT-S+/16已完成：最佳scene-guard验证mIoU=0.7863238948（epoch54），完整重算一致；官方测试集1=0.673862，高于A00的0.669362，成为当前最高官方成绩。
- B01为只用官方数据的下一对照：DINOv3结构不变，换回A00原始split和对应train-only权重。是否已经启动尚待用户确认。
- 参赛者报告已于2026-09-08获得组委会关于“可使用外部有标注数据训练”的明确回复；
  原始回复应由参赛者留存，仓库记录不能代替比赛方证据。此前外部数据禁用判断已失效。
- C00完成：LoveDA阶段最佳epoch9、mIoU=0.5521240650；迁移到AIC后最佳epoch55、
  验证mIoU=0.8147344508，只重置9类分类头的weight/bias。官方测试成绩待登记。
- C01无卡准备已实现：直接从UAVid++ RGB/标签ZIP生成官方1088方形瓦片，固定保留
  原Train/Val序列，映射到AIC 9类空间，严格不解码或训练UAVid++ Test。训练尚未启动。
...
1. 冻结B00和C00输出，不覆盖已有检查点；补录C00官方测试成绩后再做最终判断。
2. 在无卡实例上传并核验正确的`UAVid_rgb_only.zip`与`UAVid++_labels.zip`，运行
   `tools/prepare_uavidplusplus.py`、完整数据检查和train-only权重生成；到此停止。
3. GPU可用后，C01先从C00 LoveDA最佳8类检查点初始化UAVid++ 9类适配阶段；必须仅
   重置分类头两个张量。随后从其最佳检查点初始化AIC官方训练阶段。
4. C01与C00的AIC阶段必须使用同一官方split、权重、60轮与其余超参数；唯一处理差异
   是中间是否经过UAVid++，因此可直接比较验证与官方成绩。
5. 工作区`run_d03.ps1`和`tools/audit_label_consistency.py`为用户未跟踪文件，本次未修改或删除。
