# C00 LoveDA 两阶段迁移（历史执行指南）

> 2026-09-09更新：参赛者报告比赛方已明确允许外部有标注数据训练，且C00已经完成。
> 本页保留最初按“外部数据不可提交”假设设计的防护与旧命令，仅用于追溯，**不要重新
> 执行**。当前结果见`docs/EXPERIMENTS.md`，下一实验按`docs/C01_UAVIDPLUSPLUS_GUIDE.md`。

以下为C00启动前的历史设计，当时尚未获得上述比赛方回复。
