...
模型实现见 `src/uavseg/model.py`。MiT-B2 编码器约 24.7M 参数，本地解码器约 0.55M 参数，总规模约 25.3M；训练启动时应以实际环境统计值为准。
...
- Balanced Boundary BCE 对边界正样本动态加权并约束类别交界处；与 Ignore 类相邻的位置不产生边界监督。
