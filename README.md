...
参赛者于2026-09-08报告已向组委会确认：允许使用外部有标注数据训练。该口径必须以
参赛者保存的原始回复作为最终合规依据；仓库不声称独立核验了私聊内容。C00已完成
LoveDA预适配和官方数据微调；C01在相同LoveDA起点与官方微调设置之间插入UAVid++
低空场景适配。C01只使用UAVid++官方Train/Val，不训练其Test，且不混用同源的UAVid、
UAVid+旧标签。完整流程见`docs/C01_UAVIDPLUSPLUS_GUIDE.md`。
